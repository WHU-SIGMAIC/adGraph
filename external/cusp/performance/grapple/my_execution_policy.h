...
public:
    timer(void)
    {
        if((error = hipEventCreate(&start)) != hipSuccess)
            throw cusp::runtime_exception("hipEventCreate failed (start)");
        if((error = hipEventCreate(&end)) != hipSuccess)
            throw cusp::runtime_exception("hipEventCreate failed (end)");
    }

    ~timer(void)
    {
        if((error = hipEventDestroy(start)) != hipSuccess)
            throw cusp::runtime_exception("hipEventDestroy failed (start)");
        if((error = hipEventDestroy(end)) != hipSuccess)
            throw cusp::runtime_exception("hipEventDestroy failed (end)");
    }

    void Start(void)
    {
        if((error = hipEventRecord(start, 0)) != hipSuccess)
        {
            std::cout << error << std::endl;
            throw cusp::runtime_exception("hipEventRecord failed (start)");
        }
    }

    void Stop(void)
    {
        if((error = hipEventRecord(end, 0)) != hipSuccess)
            throw cusp::runtime_exception("hipEventRecord failed (end)");
        if((error = hipEventSynchronize(end)) != hipSuccess)
            throw cusp::runtime_exception("hipEventSynchronize failed");
    }

    float milliseconds_elapsed(void)
    {
        float elapsed_time;
        if((error = hipEventElapsedTime(&elapsed_time, start, end)) != hipSuccess)
            throw cusp::runtime_exception("hipEventSynchronize failed");
