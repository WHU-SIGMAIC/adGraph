...
int main(void)
{
    ...
    int   * device_I;
    hipMalloc(&device_I, 10 * sizeof(int));
    int   * device_J;
    hipMalloc(&device_J, 10 * sizeof(int));
    float * device_V;
    hipMalloc(&device_V, 10 * sizeof(float));

    // allocate device memory for x and y arrays
    float * device_x;
    hipMalloc(&device_x, 4 * sizeof(float));
    float * device_b;
    hipMalloc(&device_b, 4 * sizeof(float));

    // copy raw data from host to device
    hipMemcpy(device_I, host_I, 10 * sizeof(int),   hipMemcpyHostToDevice);
    hipMemcpy(device_J, host_J, 10 * sizeof(int),   hipMemcpyHostToDevice);
    hipMemcpy(device_V, host_V, 10 * sizeof(float), hipMemcpyHostToDevice);
    hipMemcpy(device_x, host_x,  4 * sizeof(float), hipMemcpyHostToDevice);
    hipMemcpy(device_b, host_b,  4 * sizeof(float), hipMemcpyHostToDevice);
    ...
    hipMemcpy(host_x, device_x,  4 * sizeof(float), hipMemcpyDeviceToHost);
