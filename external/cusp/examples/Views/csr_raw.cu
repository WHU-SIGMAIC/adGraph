...
int main(void)
{
    ...
    int   * device_Ap;
    hipMalloc(&device_Ap, 5 * sizeof(int));
    int   * device_Aj;
    hipMalloc(&device_Aj, 6 * sizeof(int));
    float * device_Ax;
    hipMalloc(&device_Ax, 6 * sizeof(float));

    // allocate device memory for x and y arrays
    float * device_x;
    hipMalloc(&device_x, 3 * sizeof(float));
    float * device_y;
    hipMalloc(&device_y, 4 * sizeof(float));
