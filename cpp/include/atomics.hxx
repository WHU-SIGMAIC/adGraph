...
// atomicAdd for double starts with sm_60
    ...
    static __inline__ __device__ float atomicFPMin(float *addr, float val)
    {
        ...
        {
            assumed = old;
            old = __int_as_float(atomicCAS((int *)addr, __float_as_int(assumed), __float_as_int(min(val, assumed))));
    ...
    static __inline__ __device__ float atomicFPMax(float *addr, float val)
    {
        ...
        {
            assumed = old;
            old = __int_as_float(atomicCAS((int *)addr, __float_as_int(assumed), __float_as_int(max(val, assumed))));
        } while (__float_as_int(assumed) != __float_as_int(old));
    ...
    static __inline__ __device__ float atomicFPOr(float *addr, float val)
    {
        ...
        {
            assumed = old;
            old = __int_as_float(atomicCAS((int *)addr, __float_as_int(assumed), __float_as_int((bool)val | (bool)assumed)));
        } while (__float_as_int(assumed) != __float_as_int(old));
    ...
    static __inline__ __device__ float atomicFPLog(float *addr, float val)
    {
        ...
        {
            assumed = old;
            old = __int_as_float(atomicCAS((int *)addr, __float_as_int(assumed), __float_as_int(-logf(expf(-val) + expf(-assumed)))));
        } while (__float_as_int(assumed) != __float_as_int(old));
