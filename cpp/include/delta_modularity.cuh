...
#pragma unroll
  ...
  */
                                  ...
                                  ValType *delta_modularity_vec)
  {
    ...
    if (wid < n_vertex)
    {
      ...
      for (unsigned xid = 0; xid + tid < (degree); xid += WARP_SIZE)
      {
        ...
        stride = std::min(16u, (degree) / 2u + 1u);
  ...
  max_delta_modularity_vec(const int n_vertex,
    ...
    if( i < n_vertex ){
      ...
      best_pos_ptr = thrust::max_element(thrust::hip::par, delta_modularity_vec + start, delta_modularity_vec + end);
    }

    if( i < n_vertex ){
      //printf("i: %d max: %f\n", i, (ValType)(*best_pos_ptr));
      thrust::replace_if(thrust::hip::par, delta_modularity_vec + start, delta_modularity_vec + end, not_best<ValType>(*best_pos_ptr), 0.0);
  ...
  */
  ...
  //   thrust::fill(thrust::hip::par, delta_Q_arr_ptr, delta_Q_arr_ptr + n_edges, 0.0);
