...
static void ref_coo2csr(const int *cooRowindx, int nnz, int m, int *csrRowPtr, int base = 0)
{
    ...
    for (int i = 0; i < nnz; i++)
    { // fill csrRowPtr with number of nnz per row
...
public:
    ...
    static void cpyTopo(void *dst, void *src, testTopologyType_t TType, enum hipMemcpyKind kind = hipMemcpyDefault)
    {
        ...
        if (TType == CSR_32)
        {
            ...
            dstT->nvertices = srcT->nvertices;
            dstT->nedges = srcT->nedges;
            rowSize = srcT->nvertices + 1;
            colSize = srcT->nedges;
            srcRow = srcT->source_offsets;
            dstRow = dstT->source_offsets;
            srcCol = srcT->destination_indices;
            dstCol = dstT->destination_indices;
        }
        else if (TType == CSC_32)
        {
            ...
            dstT->nedges = srcT->nedges;
            rowSize = srcT->nedges;
            colSize = srcT->nvertices + 1;
            srcRow = srcT->source_indices;
            dstRow = dstT->source_indices;
            srcCol = srcT->destination_offsets;
            dstCol = dstT->destination_offsets;
        }
        else if (TType == COO_SOURCE_32 || TType == COO_DESTINATION_32 || TType == COO_UNSORTED_32 || TType == COO_DEFAULT_32)
        {
...
TEST_P(RandomGraph, nvgraphConvertGraph)
{
    ...
    else if (srcTopoType == NVGRAPH_COO_32)
    { // COO graph is not supported
    ...
    else if (dstTopoType == NVGRAPH_COO_32)
    { // We don't support COO graphs
        ASSERT_EQ(NVGRAPH_STATUS_TYPE_NOT_SUPPORTED, status);
        return;
    }
    else
